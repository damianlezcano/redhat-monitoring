...
class AbstractCommitCollector(pelorus.AbstractPelorusExporter):
    ...
    def get_metrics_from_apps(self, apps, namespace):
        metrics = []
        urls = self.get_url_apps(apps)
        logging.info("## get_metrics_from_apps -> 0 - urls: %s", urls)
        ...
        for app in apps:
            ...
            for build in code_builds:
                try:
                    name = self.get_name(build.metadata.labels.app)
                    logging.info("## get_metrics_from_apps -> 1 - name: %s", name)
                    metric = self.get_metric_from_build(build, app, namespace, urls[name])
                except Exception:
    ...
    def get_url_apps(self, apps):
        ...
        for app in apps:
            ...
            name = self.get_name(app)

            logging.info("## get_url_apps -> 0 - name: %s", name)
    ...
    def get_name(self,name):
        name = re.sub(r'(?is)-develop.*', '', name)
        name = re.sub(r'(?is)-release.*', '', name)
        return name

    def get_metric_from_build(self, build, app, namespace, repo_url):
        try:

            logging.info("## get_metric_from_build -> 0 - app: %s, repo_url: %s", app, repo_url)
